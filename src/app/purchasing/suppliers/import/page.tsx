...
export default function SupplierImportPage() {
  ...
  function handleDownloadTemplate() {
    ...
    downloadCsv("仕入先マスタ一括登録テンプレート.csv", headers, []);
    toast.show("仕入先マスタ一括登録テンプレートをダウンロードしました", "success");
  }
  ...
  function confirmImport() {
    // プレビューの正常・警告行（エラー行・仕入先コード未入力行は除外）を仕入先マスタへ実際に反映する。
    // モードに応じて新規追加のみ／既存更新のみ／両方を出し分け、支払条件・振込先の上書きはオプションに従う。
    // 取込ページは仕入先マスタのオーナーページではないため、reload後も残るよう直接 snapshot する。
    const importable = previewData.filter((d) => d.status !== "error" && d.code.trim().length > 0);
    let applied = 0;
    for (const row of importable) {
      const existing = supplierStore.findById(row.code);
      ...
      const next: SupplierRecord = {
        ...
        email: row.email,
        monthVolume: existing?.monthVolume ?? 0,
        ytdVolume: existing?.ytdVolume ?? 0,
        unpaid: existing?.unpaid ?? 0,
        leadTime: existing?.leadTime ?? 0,
        rating: existing?.rating ?? "B",
        status: existing?.status ?? "新規",
        paymentTerms: existing && !updatePaymentTerms ? existing.paymentTerms : row.paymentTerms,
        bank: existing && !updateBankAccount ? existing.bank : row.bank,
      };
      supplierStore.upsert(next);
      applied += 1;
    }
    if (applied > 0) {
      void snapshotDomain("suppliers", supplierStore.getState());
      toast.show(`${applied} 件を ${MODE_LABEL[mode]} で取込しました`);
    } else {
      toast.show(`取込対象の仕入先がありませんでした（${MODE_LABEL[mode]}）`, "info");
    }
    setStep(1);
    setFile(null);
  }
        ...
        rows={
          detailRow
            ? [
                ...
                { label: "登録モード", value: MODE_LABEL[detailRow.mode] },
