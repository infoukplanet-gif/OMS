...
import {
  ...
  type PaymentMethod,
} from "@/lib/stores/payment-methods";
...
export default function PaymentCategoriesPage() {
  ...
  const rows = useSyncExternalStore(
    paymentMethodsStore.subscribe,
    paymentMethodsStore.getState,
    paymentMethodsStore.getState,
  );
  usePersistentStore({
    store: paymentMethodsStore,
    domain: "payment-methods",
    seed: INITIAL_PAYMENT_METHODS,
  });
  ...
  const [confirmTarget, setConfirmTarget] = useState<PaymentMethod | null>(null);
  ...
  function save() {
    ...
    if (editing) {
      ...
      const currentIds = paymentMethodsStore.getState().map((r) => {
        const m = /^PM-(\d+)$/.exec(r.id);
      ...
      paymentMethodsStore.upsert(newRecord);
      toast.show(`「${n}」を追加しました`);
    }
    setOpen(false);
  }
  function toggleEnabled(r: PaymentMethod) {
    paymentMethodsStore.upsert({ ...r, enabled: !r.enabled });
  }
  function confirmDelete() {
    if (!confirmTarget) return;
    paymentMethodsStore.remove(confirmTarget.id);
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setPrepaid(e.target.value === "pre")}
                className="w-full px-3 py-2 rounded-xl text-sm bg-white/70 border border-white/60 focus:outline-none focus:border-blue-400/60"
              >
