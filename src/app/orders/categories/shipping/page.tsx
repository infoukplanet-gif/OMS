...
import {
  ...
  type ShippingMethod,
} from "@/lib/stores/shipping-methods";
...
export default function ShippingCategoriesPage() {
  ...
  const rows = useSyncExternalStore(
    shippingMethodsStore.subscribe,
    shippingMethodsStore.getState,
    shippingMethodsStore.getState,
  );
  usePersistentStore({
    store: shippingMethodsStore,
    domain: "shipping-methods",
    seed: INITIAL_SHIPPING_METHODS,
  });
  ...
  function openEdit(r: ShippingMethod) {
    setEditing(r);
    setCode(r.code);
    setName(r.name);
    setFee(r.fee);
  ...
  function save() {
    ...
    if (editing) {
      ...
      const currentIds = shippingMethodsStore.getState().map((r) => {
        const m = /^SM-(\d+)$/.exec(r.id);
        return m ? Number(m[1]) : 0;
      });
      const nextSeq = Math.max(0, ...currentIds) + 1;
      const newRecord: ShippingMethod = {
        id: `SM-${nextSeq}`,
        code: c,
        name: n,
        fee,
      ...
      shippingMethodsStore.upsert(newRecord);
      toast.show(`「${n}」を追加しました`);
    }
    setOpen(false);
  }
  function toggleEnabled(r: ShippingMethod) {
    shippingMethodsStore.upsert({ ...r, enabled: !r.enabled });
  }
  function confirmDelete() {
    if (!confirmTarget) return;
    shippingMethodsStore.remove(confirmTarget.id);
