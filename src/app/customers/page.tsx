...
import { customerStore, INITIAL_CUSTOMERS } from "@/lib/stores/customer";
import {
  Search,
  Plus,
  ChevronLeft,
...
// allCustomers は customerStore に移動。一覧はストアから購読する。
...
export default function CustomersPage() {
  ...
  const allCustomers = useSyncExternalStore(
    (cb) => customerStore.subscribe(cb),
    () => customerStore.getState() as readonly Customer[],
    () => INITIAL_CUSTOMERS as readonly Customer[],
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {allCustomers.length} 件中 <span className="font-semibold text-gray-700">{filtered.length}</span> 件表示
          </p>
        </div>
        <div className="flex items-center gap-2">
