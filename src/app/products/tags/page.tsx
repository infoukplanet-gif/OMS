...
export default function ProductTagsPage() {
  ...
  // 永続化オーナー（このページのみ呼ぶ）
  usePersistentStore({ store: productTagsStore, domain: "product-tags", seed: INITIAL_PRODUCT_TAGS });

  // 共有ストアを購読
  const tags = useSyncExternalStore(
    productTagsStore.subscribe,
    productTagsStore.getState,
    productTagsStore.getState,
  );
  ...
  function handleSave() {
    ...
    if (editing) {
      productTagsStore.upsert({ ...editing, name: trimmed, color });
      toast.show(`「${trimmed}」を更新しました`);
    } else {
      // 新規 ID 採番: 既存 PTAG-n の n 部分の最大値 + 1
      const maxN = tags.reduce((acc, t) => {
        const m = t.id.match(/^PTAG-(\d+)$/);
        return m ? Math.max(acc, parseInt(m[1], 10)) : acc;
      }, 0);
      const newId = `PTAG-${maxN + 1}`;
      productTagsStore.upsert({ id: newId, name: trimmed, color });
      toast.show(`「${trimmed}」を追加しました`);
    }
    setModalOpen(false);
  }

  function handleDelete(tag: ProductTag) {
    if (!confirm(`「${tag.name}」を削除しますか？`)) return;
    productTagsStore.remove(tag.id);
              ...
              <div key={t.id} className={cn("flex items-center justify-between p-3 rounded-xl border", t.color)}>
                ...
                <div className="flex gap-1">
                  <button
                    type="button"
                    onClick={() => openEdit(t)}
                    className="px-2 py-0.5 rounded hover:bg-white/50 text-xs"
                  >
                    編集
                  </button>
                  <button
