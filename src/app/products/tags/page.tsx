...
const initialTags: Tag[] = [
  { id: 1, name: "新商品", color: COLORS[3].className },
  { id: 2, name: "セール", color: COLORS[0].className },
  { id: 3, name: "売れ筋", color: COLORS[5].className },
  { id: 4, name: "在庫限り", color: COLORS[1].className },
  { id: 5, name: "予約受付", color: COLORS[6].className },
  { id: 6, name: "廃番予定", color: COLORS[2].className },
];
...
export default function ProductTagsPage() {
  ...
  function openNew() {
    setEditing(null); setName(""); setColorKey(COLORS[0].key); setModalOpen(true);
  }
  function openEdit(tag: Tag) {
    setEditing(tag); setName(tag.name);
  ...
  function handleSave() {
    ...
    if (!trimmed) return toast.show("タグ名を入力してください", "error");
    const dup = tags.some((t) => t.name === trimmed && t.id !== editing?.id);
    if (dup) return toast.show(`「${trimmed}」は既に存在します`, "error");
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">独自タグの登録</h1>
        <button type="button" onClick={openNew} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium bg-blue-500/90 text-white hover:bg-blue-600/90 shadow-sm">
            ...
            {tags.map((t) => (
              <div key={t.id} className={cn("flex items-center justify-between p-3 rounded-xl border", t.color)}>
                ...
                <div className="flex gap-1">
                  <button type="button" onClick={() => openEdit(t)} className="px-2 py-0.5 rounded hover:bg-white/50 text-xs">編集</button>
                  <button type="button" onClick={() => handleDelete(t)} aria-label={`${t.name} を削除`} className="p-1 rounded hover:bg-white/50">
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="例: 新商品" className="w-full px-3 py-2 rounded-xl text-sm bg-white/70 border border-white/60 focus:outline-none focus:border-blue-400/60" autoFocus />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-2">カラー</label>
            <div className="grid grid-cols-4 gap-2">
              {COLORS.map((c) => (
                <button key={c.key} type="button" onClick={() => setColorKey(c.key)} className={cn("flex items-center justify-center p-2 rounded-xl text-sm border transition-all", c.className, colorKey === c.key ? "ring-2 ring-blue-500/60 ring-offset-2 ring-offset-white/60" : "")}>
                  {c.label}
                </button>
              ))}
            </div>
