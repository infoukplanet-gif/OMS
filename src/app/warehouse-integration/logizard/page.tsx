...
export default function LogizardPage() {
  ...
  function handleConnectionTest() {
    ...
    setTimeout(() => {
      ...
      setLastChecked(ts);
      toast.show("接続テスト完了：ロジザードZEROサーバー正常応答", "success");
    }, 1500);
  }
  ...
  function handleSave() {
    ...
    setTimeout(() => {
      setSaving(false);
      setSaved(true);
      toast.show("ロジザード連携を保存しました", "success");
  ...
  function handleHistoryRefresh() {
    ...
    setTimeout(() => {
      ...
      setHistoryEvents((prev) => [
        { time: ts, action: "在庫同期", direction: "ロジザード → OMS", count: 8423, status: "成功" },
        ...prev,
      ]);
      toast.show("同期履歴を再読込しました", "info");
    }, 1000);
  }
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <GlassCard className="p-4">
          ...
          <div className="text-2xl font-bold text-blue-600 mt-1">{historyEvents.reduce((s, e) => s + e.count, 0).toLocaleString()}</div>
        </GlassCard>
        <GlassCard className="p-4">
