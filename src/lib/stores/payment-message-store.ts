 ...
 * 支払方法別メッセージ 共有マスタストア
 *
 * 支払方法ごとの注文確認メール・出荷通知・伝票印字文面の CRUD を
 * 画面横断で共有する。永続化（domain: "payment-message-settings"）の
 * 正規オーナーページは src/app/settings/payment-message/page.tsx
 */
...
export const INITIAL_PAYMENT_MESSAGES: PaymentMessage[] = [
  {
    id: "pm-credit",
    method: "クレジットカード",
...
/** クライアントセッション内で共有される単一の PaymentMessageStore インスタンス */
export const paymentMessageStore: MasterStore<PaymentMessage> =
  createMasterStore<PaymentMessage>(INITIAL_PAYMENT_MESSAGES);
