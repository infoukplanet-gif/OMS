...
/** 在庫マスタから倉庫一覧を抽出。order-form の倉庫プルダウンと整合させる。 */
const WAREHOUSES = Array.from(new Set(INITIAL_INVENTORY.map((r) => r.warehouse)));
...
type Item = {
  ...
  price: number;
  qty: number;
  /** 引当倉庫（INITIAL_INVENTORY と整合）。空文字は未選択。 */
  warehouse: string;
};
...
export function OrderForm({ mode }: OrderFormProps) {
  ...
  function validate(): string | null {
    ...
    const noSku = items.find((i) => !i.code.trim());
    if (noSku) return "商品コード（SKU）を入力してください（引当に必須）";
    const noWarehouse = items.find((i) => !i.warehouse.trim());
    if (noWarehouse) return "引当倉庫を選択してください";
    return null;
  }

  /** 次の受注番号を生成。store 内の最大番号 + 1（簡易採番）。 */
  function nextOrderId(): string {
    const all = orderStore.getState();
    const nums = all
      .map((o) => {
        const m = /ORD-2026-(\d+)/.exec(o.id);
        return m ? parseInt(m[1], 10) : 0;
      })
      .filter((n) => n > 0);
    const max = nums.length === 0 ? 8851 : Math.max(...nums);
  ...
  function save() {
    ...
    for (const i of items) {
      ...
      if (existing === undefined) {
        allocationMap.set(key, { sku: i.code, warehouse: i.warehouse, qty: i.qty });
      } else {
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-gray-800 flex items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 text-xs font-normal text-gray-500">
              <Warehouse className="h-3 w-3" />SKU + 倉庫 で在庫引当に直結
            </span>
          </h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white/50">
                ...
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 w-40">引当倉庫</th>
            ...
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={7} className="px-3 py-6 text-center text-gray-500 text-xs">商品明細がありません。「商品を追加」から行を追加してください。</td></tr>
              ) : items.map((i) => (
                <tr key={i.id} className="border-t border-white/30">
                  <td className="px-3 py-2">
                    <input
                      value={i.code}
                      list="order-form-sku-list"
                      onChange={(e) => selectSku(i.id, e.target.value)}
                      placeholder="WEP-001"
                      className="h-7 w-full px-2 rounded-lg text-xs bg-white/50 border border-white/50 font-mono"
                    />
                  </td>
                  <td className="px-3 py-2">
                  ...
                  <td className="px-3 py-2">
                    <select
                      value={i.warehouse}
                      onChange={(e) => updItem(i.id, { warehouse: e.target.value })}
